...
**(Edited: Frontend requirement removed — backend-only project)**
...
- Tickets have a status field: open → in-progress → closed (stored and correct; doesn't need to be changeable through the UI yet — ~~UI~~ **API** is enough)
...
- Status can actually be changed through the app itself — ~~through the app itself~~ **via a documented API endpoint (e.g. tested through `/docs`)**, not just edited in the database
- ~~Basic styling — no unstyled raw HTML forms on defense day~~ **(removed — no frontend, so no styling requirement)**
- Basic error handling — invalid input shows a real message, not a blank screen or a raw server error (still applies — this now means a proper API error response, not a UI message)
...
- **Frontend / UI of any kind (added — project is backend/API-only)**
...
| ~~Frontend~~ | ~~UI pages, API calls~~ | ~~Ticket list page + create-ticket form, wired to the real API~~ **(role removed)** |
...
| B1 — True MVP | Ticket end-to-end: create one, list them — **confirmed via `/docs`**, ~~on a barebones frontend~~ — completes True MVP (Section 2) | Backend: unit tests for `create_ticket()`, unit tests for `list_tickets()`, written before or right alongside the real function | Does creating a ticket via the API actually show up on a follow-up `GET`? |
| B2 — Status through the UI → **Status via API** | Status can be changed through a real endpoint, not just the database — first half of Presentable | unit tests for the status-update Service function, same habit as B1 | Can a ticket move from open → closed through a documented API call, not the database? |
...
|---|---|---|---|
| B0 — Setup | Scaffold the FastAPI app (empty `repositories/`, `services/`, `controllers/` folders, a `main.py` that runs); confirm `uvicorn` boots locally | Install/confirm MySQL running locally; create the empty project database | ~~Scaffold the frontend project...~~ **(removed)** |
| B1 — True MVP | Build Model → Repository → Service → Controller for `create_ticket` and `list_tickets`; write the two required unit tests; confirm both work via `/docs` | Finalize the agreed Ticket fields (id, subject, description, status, created_at) and draft the ERD (`docs/erd.png`); create the real MySQL table matching it | ~~Build the barebones ticket list page...~~ **(removed)** |
| B2 — Status API | Add the status-update endpoint + Service function, with its unit test | Confirm the status column cleanly supports all three values (migration if needed) | ~~Add a status-change control to the UI...~~ **(removed)** |
| B3 — Audit + polish | Fill any missing unit/integration tests found in the audit; add real error handling (validation messages, proper 404s) instead of raw 500s | Verify the live database schema matches the ERD exactly; fix any drift | ~~Basic styling pass...~~ **(removed)** |
| B4 — Submission prep | Write the API section of the README; be ready to explain/modify any layer live | Finalize the ERD/UML to match the shipped code exactly | ~~Write the UI section of the README...~~ **(removed)** |

If B0–B3 finish early, everyone pulls stretch items from Section 2 in Backend/Database order of dependency (e.g. the users table before the role-check logic) — nobody starts a stretch item before B3 is done.

---
**Summary of changes:** All frontend deliverables, the Frontend role, and UI-facing language removed throughout. "Through the app/UI" replaced with "via API / through `/docs`" wherever it appears, since the API itself (tested through FastAPI's auto-generated `/docs` interface) is now the full deliverable — there's no separate UI layer consuming it.